...
JNIEXPORT void JNICALL
Java_com_example_cc000033_learnffmpeg_VideoUtils_decode(JNIEnv *env, jclass type, jstring input_,
                                                        jstring output_) {
    //需要转码的视频文件(输入的视频文件)
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0)
    {
        ...
        if (packet->stream_index == v_stream_idx)
        {
            ...
            if (got_picture)
            {
                ...
                //5 输入画面的高度
                sws_scale(sws_ctx, reinterpret_cast<const uint8_t *const *>(pFrame->data), pFrame->linesize, 0, pCodecCtx->height,
